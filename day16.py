...
def solve_part2(line: str, phase_cnt: int) -> int:
    ...
    # took some hints to look at the output from the first part
    # after each phase and it became apparent that the back half
    # of the numbers always represented a sum values from the back mod 10.
    # given that the offset, put the values we cared about in the
    # back half, don't need to worry about the pattern logic in the
    # first half, so just focused on processing the pieces after the
    # offset value in reverse for each phase transformation.
    offset = int(line[0:7])
    isignal = isignal[offset:]
    osignal = [0] * len(isignal)
    for _ in range(phase_cnt):
        total = 0
        for idx in range(len(isignal)-1,-1,-1):
            total += isignal[idx]
            osignal[idx] = total % 10
...
assert solve_part2(data, 100) == "55005000"
